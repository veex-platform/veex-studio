...
const templates = [
    ...
    },
    {
        ...
        description: 'Read BME280 sensor and post to REST API.',
        icon: <Activity size={14} />,
        nodes: [
            { id: '1', type: 'input', data: { label: 'Boot Sequence' }, position: { x: 250, y: 0 }, className: 'bg-white !text-slate-900 border-none rounded-lg px-6 py-3 font-bold shadow-xl !w-[160px] text-center text-[10px]' },
            { id: '2', type: 'action', data: { label: 'Read BME280', action: 'read', type: 'sensor.bme280', params: { addr: '0x76' } }, position: { x: 250, y: 100 } },
            { id: '3', type: 'action', data: { label: 'HTTP Post', action: 'post', type: 'comm.http', params: { url: 'https://api.veex.io/data' } }, position: { x: 250, y: 200 } },
        ],
        edges: [
            { id: 'e1-2', source: '1', target: '2', animated: true, style: { stroke: '#475569', strokeWidth: 1, strokeDasharray: '5,5' } },
            { id: 'e2-3', source: '2', target: '3', animated: true, style: { stroke: '#475569', strokeWidth: 1, strokeDasharray: '5,5' } },
...
const categories = [
    ...
    },
    {
        ...
        items: [
            ...
            { type: 'action', label: 'UART Read', capability: 'platform.uart', icon: <MessageSquare size={14} />, params: { baud: '115200' }, action: 'read' },
        ]
    },
    {
        id: 'cloud',
    ...
    return (
        <div className="w-[260px] shrink-0 border-r border-white/5 bg-[#131722]/40 backdrop-blur-xl flex flex-col h-full z-40">
            ...
            <div className="flex-1 overflow-y-auto py-4 scrollbar-hide">
                <h2 className="px-5 text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4">Industrial Templates</h2>
                ...
                <h2 className="px-5 text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4">Capabilities</h2>
                            ...
                            <div className="mt-1 pb-2">
                                {cat.items.map((item) => (
                                    ...
                                    >
                                        ...
                                        <span className="text-[11px] text-slate-400 group-hover:text-slate-200 transition truncate">
                                            {item.label}
                                        </span>
                                    </div>
                                ))}
            ...
            <div className="p-6 text-center border-t border-white/5 bg-slate-950/20">
                <p className="text-[9px] text-slate-600 uppercase tracking-widest leading-loose">
                    Drag items to canvas<br />to define logic
                </p>
            </div>
        </div>
    );
